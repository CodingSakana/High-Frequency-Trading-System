    ...
    auto TCPSocket::sendAndRecv() noexcept -> bool {
        ...
        if (read_size > 0) {
            ...
            logger_.log("%:% %() % read socket:% len:% utime:% ktime:% diff:%\n", __FILE__, __LINE__, __FUNCTION__,
                        Common::getCurrentTimeStr(&time_str_), socket_fd_, next_rcv_valid_index_, user_time,
        ...
        if (next_send_valid_index_ > 0) {
            ...
            const auto n =
                ::send(socket_fd_, outbound_data_.data(), next_send_valid_index_, MSG_DONTWAIT | MSG_NOSIGNAL);
