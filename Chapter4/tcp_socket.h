...
namespace Common
{
    ...
    struct TCPSocket {
        ...
        /// Called to publish outgoing data from the buffers as well as check for and callback if data is available in
        /// the read buffers.
        ...
        std::vector<char> outbound_data_; // 储存要发送的数据
        size_t next_send_valid_index_ = 0;
        std::vector<char> inbound_data_; // 储存刚刚读取的数据
