...
namespace Common
{
/// Represents the configuration for a socket, including IP, interface, port, and protocol type.
struct SocketCfg {
    std::string ip_;
    std::string iface_;
    int port_ = -1;
...
/// Add / Join membership / subscription to the multicast stream specified and on the interface
/// specified.
...
/* 这里的和网站上的完全不一样 */
/// Create a TCP / UDP socket to either connect to or listen for data on or listen for connections
...
[[nodiscard]] inline auto createSocket(Logger& logger, const SocketCfg& socket_cfg) -> int {
    ...
    /* __FILE__ 是当前源文件的名称，__LINE__ 是当前代码所在的行号 */
    logger.log("%:% %() % cfg:%\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str),
               socket_cfg.toString());

    /* AI_PASSIVE 表示可以用于 bind() 的地址 */ /* AI_NUMERICHOST 禁止 DNS，host 必须是数字 IP */ /* AI_NUMERICSERV 禁止服务名解析，port 必须是数字 */
    ...
    for (addrinfo* rp = result; rp; rp = rp->ai_next) {
        ...
        if (socket_cfg.is_listening_) { // allow re-using the address in the call to bind()
            ASSERT(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&one), sizeof(one)) ==
                       0,
