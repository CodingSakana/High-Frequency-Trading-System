...
/**
 * 这里是 Order Gateway Client
 * 主要职责是和 ME 头的 Order Gateway Server 进行通信
 * 接收的 data 先由 TCPSocket::sendAndRecv() 存入 inbound_data 缓冲区
 * 然后由 OrderGateway::recvCallback(TCPSocket* socket, Nanos rx_time)
 * 存入 LFQueue 等待 TE 来取
 */
...
public:
    OrderGateway(ClientId client_id, Exchange::ClientRequestLFQueue* client_requests,
                 Exchange::ClientResponseLFQueue* client_responses, std::string ip, const std::string& iface, int port);
    ...
    auto start() {
        ...
        ASSERT(tcp_socket_.connect(ip_, iface_, port_, false) >= 0,
               "Unable to connect to ip:" + ip_ + " port:" + std::to_string(port_) + " on iface:" + iface_ +
...
private:
    ...
    /// Lock free queue on which we consume client requests from the trade engine and forward them to the exchange's
    /// order server.
    Exchange::ClientRequestLFQueue* outgoing_requests_ = nullptr;

    /// Lock free queue on which we write client responses which we read and processed from the exchange, to be consumed
    /// by the trade engine.
    ...
    /// Sequence numbers to track the sequence number to set on outgoing client requests and expected on incoming client
    /// responses.
