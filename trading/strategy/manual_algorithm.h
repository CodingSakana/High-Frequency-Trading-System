...
private:
    ...
    void cliLoop() {
        std::string line;
        while (keep_running_.load() && std::getline(std::cin, line)) { // 也可以替换成 TCP recv
            ...
            if (cmd == "BUY" || cmd == "SELL") {
                ...
                if (side == Side::BUY)
                    order_manager_->moveOrders(ticker, px, Price_INVALID, qty);
                else
                    order_manager_->moveOrders(ticker, Price_INVALID, px, qty);
            } else if (cmd == "CANCEL") {
                int ticker;
                std::string sideStr;
                iss >> ticker >> sideStr;
                Side oppSide = (sideStr == "BUY") ? Side::SELL : Side::BUY;
